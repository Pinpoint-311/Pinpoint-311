...
class SeeClickFixConnector(BaseConnector):
            ...
            if key in supplied:
                ...
                if qtype in _CHOICE_TYPES:
                    ...
                    if unknown:
                        ...
                        continue
                    if qtype == "multivaluelist" and not isinstance(value, list):
                        # The API wants an array for a multi-choice question; a
                        # bare scalar can come back as a 422.
                        value = [value]
                answers[key] = value
                continue
    ...
    async def test_connection(self) -> Dict[str, Any]:
        """Sign in, then check whether a report could actually be filed.
        ...
        Signing in is half the question. A connection can be perfectly
        authenticated and still unable to file a report -- no request type set,
        or one that asks a required question nobody can answer. Those come back
        as `warnings` alongside the pass, not instead of it.
        """
        ...
        result: Dict[str, Any] = {"ok": True, "verified": True, "detail": detail}
        if warnings:
