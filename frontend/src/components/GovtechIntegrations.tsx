...
import {
    StatusPill, CapabilityTile, Action, Switch, ResultNote, announceStatus, hasAlert,
} from './capabilityUI';
...
export default function GovtechIntegrations() {
    ...
    const runFinishTest = async (platform: IntegrationPlatform, saved?: IntegrationConfig | null) => {
        ...
        setTestResult(null);
        setShowTechnical(false);
        announceStatus(`Checking the connection to ${platform.name}…`);
        try {
            const result = await api.testIntegration(existing.id);
            setTestResult(result);
            announceStatus(result.ok
    ...
    const handleToggle = async (existing: IntegrationConfig, name: string) => {
        ...
        try {
            ...
            announceStatus(`${name} connection turned ${existing.enabled ? 'off' : 'on'}.`);
            await load();
        } catch (err: any) {
    ...
    const handleCardTest = async (existing: IntegrationConfig, name: string) => {
        ...
        announceStatus(`Checking the connection to ${name}…`);
        ...
        try {
            ...
            announceStatus(`${name} check failed. ${detail}`);
        } finally {
            setBusyFor(existing.id, null);
        }
    };
                    ...
                    return (
                        ...
                        >
                            ...
                            >
                                ...
                                {/* Always the pill, never a bare coloured glyph.
                                ...
                                {!existing ? (
                                    ...
                                    <>
                                        ...
                                        <Action size="sm" onClick={() => toggleLogs(existing)} chevron
                                            ...
                                            controls={`conn-logs-${platform.platform}`}>
                                            Activity
                                        </Action>
