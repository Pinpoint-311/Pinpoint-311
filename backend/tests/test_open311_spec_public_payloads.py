...
def test_a_staff_comment_is_attributed_to_staff_and_nothing_more(
    ...
    # The classification and the projection are one call: `redacted()` is the
    # only way this model is built, so a public route cannot assemble one field
    # by field and get the redaction wrong.
    payload = schemas.PublicRequestCommentResponse.redacted(comment).model_dump()
    assert (payload["author_type"], payload["username"]) == (expected_type, expected_name)
