                ...
                {cap === 'ai' && active && (() => {
                    ...
                    return (
                        <div>
                            ...
                            {/* Search filter for AI models */}
                            {models.length > 4 && (
                                <div className="relative mb-2">
                                    <Search className="w-3.5 h-3.5 absolute left-3 top-2.5 text-white/40 pointer-events-none" aria-hidden="true" />
                                    <input
                                        type="text"
                                        value={modelSearchQuery}
                                        onChange={e => setModelSearchQuery(e.target.value)}
                                        placeholder={`Search ${models.length} available models (e.g. claude, flash, gemma, llama, 3.6)...`}
                                        className="w-full bg-white/[0.04] border border-white/10 rounded-xl pl-9 pr-3 py-1.5 text-xs text-white placeholder-white/40 focus:outline-none focus:border-primary-400/60 transition-colors"
                                    />
                                </div>
                            )}
                            {/* Tiles, with live search filtering for large model lists */}
                            {(() => {
                                ...
                                return (
                                    <>
                                        {filtered.length === 0 ? (
                                            <div className="py-4 text-center text-xs text-white/50 bg-white/[0.02] border border-white/5 rounded-xl">
                                                No models matching &ldquo;{modelSearchQuery}&rdquo;
                                            </div>
                                        ) : (
                                            <div className="max-h-[320px] overflow-y-auto pr-1 custom-scrollbar grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2" role="radiogroup" aria-label="AI model">
                                                {filtered.map(m => {
