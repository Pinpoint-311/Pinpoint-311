...
@pytest.mark.asyncio
async def test_a_scalar_multivaluelist_answer_is_sent_as_a_list(monkeypatch):
    """The API wants an array for a multi-choice question; a bare scalar can
    422. A town that typed one value shouldn't have to know JSON list syntax."""
    seen = route(monkeypatch, questions=[
        {"primary_key": "400", "question": "Surface?", "question_type": "multivaluelist",
         "response_required": True,
         "select_values": [{"key": "Brick", "name": "Brick"}, {"key": "Wood", "name": "Wood"}]},
    ])
    await scf({"request_type_id": "657", "answers": {"400": "Brick"}}).push_request(PAYLOAD)
    assert seen["create_body"]["answers"]["400"] == ["Brick"]
